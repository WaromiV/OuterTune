...
@RequiresApi(19)
internal object DocumentsContractApi19Ot {
    ...
    fun isVirtual(context: Context, self: Uri): Boolean {
        ...
        return (getFlags(context, self) and FLAG_VIRTUAL_DOCUMENT.toLong()) != 0L
    }
    ...
    fun exists(context: Context, self: Uri): Boolean {
        ...
        return try {
            c = resolver.query(
                self, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID), null, null, null
            )
            c!!.count > 0
        } catch (e: Exception) {
    ...
    private fun queryForString(
        context: Context, self: Uri, column: String, defaultValue: String?
    ): String? {
        val resolver = context.contentResolver
        var c: Cursor? = null
        return try {
    ...
    private fun queryForInt(context: Context, self: Uri, column: String, defaultValue: Int): Int {
        return queryForLong(context, self, column, defaultValue.toLong()).toInt()
    }

    private fun queryForLong(
        context: Context, self: Uri, column: String, defaultValue: Long
    ): Long {
        val resolver = context.contentResolver
        var c: Cursor? = null
        return try {
